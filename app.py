...
.main .block-container{{padding:0 2rem 4rem!important;max-width:1300px!important;margin:0 auto!important}}
...
.sv-card{{
  ...
  box-shadow:0 4px 24px rgba(0,0,0,{SHADOW})}}
.sv-card2{{
  background:{SURF2};border:1px solid {BORDER};border-radius:14px;
...
.sv-tile{{
  ...
  box-shadow:0 2px 16px rgba(0,0,0,{SHADOW});
  position:relative;overflow:hidden}}
.sv-tile::before{{
  content:'';position:absolute;top:0;left:0;right:0;height:3px;
  background:linear-gradient(90deg,{GRAD1},{GRAD2})}}
.sv-tile .v{{
  font-size:30px;font-weight:800;color:{AC};font-family:'Sora',serif;
  letter-spacing:-0.02em}}
.sv-tile .l{{
  font-size:11px;color:{TX3};text-transform:uppercase;
...
.sv-ring-inner{{
  background:{SURF};border-radius:50%;width:152px;height:152px;
  display:flex;flex-direction:column;align-items:center;justify-content:center}}
.sv-score{{
  font-size:58px;font-weight:800;font-family:'Sora',sans-serif;line-height:1;
  background:linear-gradient(135deg,{GRAD1},{GRAD2});
  -webkit-background-clip:text;-webkit-text-fill-color:transparent}}
.sv-score-label{{
  font-size:11px;color:{TX3};font-weight:600;
...
.sv-topbar-wrap{{
  ...
  box-shadow:0 2px 20px rgba(0,0,0,{SHADOW})}}
.sv-topbar-inner{{
  max-width:1300px;margin:0 auto;padding:10px 2rem;
...
.sv-progress-fill{{height:100%;border-radius:50px;background:linear-gradient(90deg,{GRAD1},{GRAD2});transition:width .6s cubic-bezier(.34,1.56,.64,1)}}

/* ── FEATURE BADGE ── */
.sv-feature{{display:inline-flex;align-items:center;gap:6px;background:{ACBG};color:{AC};border:1px solid {AC}30;border-radius:8px;padding:4px 12px;font-size:12px;font-weight:600;margin-right:8px;margin-bottom:8px}}
...
/* ── HERO ── */
.lp-hero{{
  min-height:90vh;background:{HERO};
  position:relative;overflow:hidden;
...
.lp-orb-b{{position:absolute;width:550px;height:550px;background:radial-gradient(circle,rgba(129,140,248,0.12) 0%,transparent 65%);border-radius:50%;bottom:-180px;left:-180px;pointer-events:none;animation:lp-float 11s ease-in-out infinite reverse}}
.lp-orb-c{{position:absolute;width:300px;height:300px;background:radial-gradient(circle,rgba(52,211,153,0.08) 0%,transparent 65%);border-radius:50%;top:40%;left:40%;pointer-events:none;animation:lp-float 7s ease-in-out infinite 2s}}
@keyframes lp-float{{0%,100%{{transform:translateY(0) scale(1)}}50%{{transform:translateY(-35px) scale(1.04)}}}}
.lp-grid{{position:absolute;top:0;left:0;right:0;bottom:0;background-image:linear-gradient(rgba(56,189,248,0.04) 1px,transparent 1px),linear-gradient(90deg,rgba(56,189,248,0.04) 1px,transparent 1px);background-size:60px 60px;pointer-events:none}}

.lp-eyebrow{{display:inline-flex;align-items:center;gap:8px;background:rgba(56,189,248,0.12);border:1px solid rgba(56,189,248,0.25);border-radius:50px;padding:6px 18px;font-size:12px;font-weight:700;color:#38bdf8;letter-spacing:0.08em;text-transform:uppercase;margin-bottom:24px}}
.lp-headline{{font-family:'Sora',sans-serif;font-size:clamp(36px,5vw,70px);font-weight:800;line-height:1.05;letter-spacing:-0.03em;color:#ffffff;margin-bottom:20px}}
.lp-headline span{{background:linear-gradient(135deg,#38bdf8,#818cf8,#34d399);-webkit-background-clip:text;-webkit-text-fill-color:transparent;background-size:200%;animation:gradient-shift 4s ease infinite}}
@keyframes gradient-shift{{0%{{background-position:0%}}50%{{background-position:100%}}100%{{background-position:0%}}}}
.lp-sub{{font-size:18px;color:rgba(255,255,255,0.6);max-width:540px;line-height:1.7;margin-bottom:36px;font-weight:400}}
...
.lp-section-tag{{display:inline-block;background:{ACBG};color:{AC};border:1px solid {AC}30;border-radius:6px;padding:4px 14px;font-size:11px;font-weight:700;letter-spacing:.1em;text-transform:uppercase;margin-bottom:12px}}
.lp-section-title{{font-family:'Sora',sans-serif;font-size:clamp(26px,3vw,42px);font-weight:800;color:{TX};letter-spacing:-0.02em;margin-bottom:10px;line-height:1.1}}
...
.lp-feat-title{{font-family:'Sora',sans-serif;font-size:17px;font-weight:700;color:{TX};margin-bottom:8px}}
...
.lp-step-num{{min-width:44px;height:44px;border-radius:50%;background:linear-gradient(135deg,{GRAD1},{GRAD2});display:flex;align-items:center;justify-content:center;font-family:'Sora',sans-serif;font-size:15px;font-weight:800;color:#fff;box-shadow:0 4px 14px rgba(56,189,248,0.35);flex-shrink:0}}
.lp-step-title{{font-family:'Sora',sans-serif;font-size:16px;font-weight:700;color:{TX};margin-bottom:4px}}
...
/* ── CTA BAND ── */
.lp-cta-band{{background:linear-gradient(135deg,{GRAD1}18,{GRAD2}18);border:1px solid {GRAD1}30;border-radius:24px;padding:60px 40px;text-align:center;margin:0 2rem 60px;position:relative;overflow:hidden}}
.lp-cta-band::before{{content:'';position:absolute;top:-100px;left:50%;transform:translateX(-50%);width:600px;height:300px;background:radial-gradient(circle,{GRAD1}12 0%,transparent 70%);pointer-events:none}}
...
.stDataFrame th{{background:{SURF2}!important;color:{TX}!important;font-weight:600!important}}
.stDataFrame td{{color:{TX}!important}}
...
[data-testid="stFormSubmitButton"]>button{{
  ...
  font-size:15px!important;font-family:'Sora',sans-serif!important;
...
def make_bar_bytes(inp):
    ...
    vals=[min(100,inp["Hours_Studied"]/8*100),min(100,inp["Attendance"]),min(100,inp["Previous_Scores"]),min(100,inp["Sleep_Hours"]/8*100)]
    ...
    for bar,val in zip(bars,vals):
        ax.text(bar.get_width()+1.2,bar.get_y()+bar.get_height()/2,f"{val:.0f}%",va="center",ha="left",fontsize=9,color="#0f1c2e",fontweight="bold")
    ...
    ax.tick_params(axis="y",labelsize=8,colors="#334e68"); ax.tick_params(axis="x",labelsize=8,colors="#334e68")
    ax.spines[["top","right"]].set_visible(False); ax.spines[["left","bottom"]].set_color("#dce4ef")
...
def generate_pdf(user_data, result, inp):
    ...
    C_BLUE=colors.HexColor("#0284c7"); C_BLUE2=colors.HexColor("#6366f1")
    C_DARK=colors.HexColor("#0f1c2e"); C_MID=colors.HexColor("#4a6080")
    C_LIGHT=colors.HexColor("#f0f4f8"); C_BORDER=colors.HexColor("#dce4ef")
    C_GREEN=colors.HexColor("#059669"); C_ORANGE=colors.HexColor("#d97706")
    C_RED=colors.HexColor("#dc2626"); C_WHITE=colors.white
    ss=getSampleStyleSheet()
    def sty(name,**kw): return ParagraphStyle(name,parent=ss["Normal"],**kw)
    S_TITLE=sty("Title",fontName="Helvetica-Bold",fontSize=22,textColor=C_WHITE,spaceAfter=2,leading=28)
    S_SUB=sty("Sub",fontName="Helvetica",fontSize=10,textColor=colors.HexColor("#b0c8e0"),spaceAfter=0,leading=14)
    S_H1=sty("H1",fontName="Helvetica-Bold",fontSize=13,textColor=C_BLUE,spaceBefore=10,spaceAfter=4,leading=16)
    S_H2=sty("H2",fontName="Helvetica-Bold",fontSize=11,textColor=C_DARK,spaceBefore=6,spaceAfter=2,leading=14)
    S_BODY=sty("Body",fontName="Helvetica",fontSize=9,textColor=C_MID,spaceAfter=3,leading=13)
    S_SCORE=sty("Score",fontName="Helvetica-Bold",fontSize=42,textColor=C_BLUE,alignment=TA_CENTER,leading=48)
    S_GRADE=sty("Grade",fontName="Helvetica-Bold",fontSize=16,textColor=C_DARK,alignment=TA_CENTER,spaceAfter=6)
    S_CENTER=sty("Center",fontName="Helvetica",fontSize=9,textColor=C_MID,alignment=TA_CENTER)
    S_TIP=sty("Tip",fontName="Helvetica",fontSize=9,textColor=C_DARK,spaceAfter=4,leading=13,leftIndent=8)
    ...
    S_FOOTER=sty("Footer",fontName="Helvetica-Oblique",fontSize=8,textColor=C_MID,alignment=TA_CENTER)
    g,desc,_=grade(result["score"]); score=result["score"]; sugs=suggestions(score,inp); usable=W-2*MARGIN
    story=[]
    header_data=[[Paragraph("ScoreVision AI",S_TITLE),Paragraph(f"Performance Report<br/>Generated: {datetime.datetime.now().strftime('%d %B %Y, %I:%M %p')}",S_SUB)]]
    header_tbl=Table(header_data,colWidths=[usable*0.55,usable*0.45])
    header_tbl.setStyle(TableStyle([("BACKGROUND",(0,0),(-1,-1),C_BLUE),("VALIGN",(0,0),(-1,-1),"MIDDLE"),("LEFTPADDING",(0,0),(0,-1),16),("RIGHTPADDING",(1,0),(1,-1),16),("TOPPADDING",(0,0),(-1,-1),16),("BOTTOMPADDING",(0,0),(-1,-1),16)]))
    story.append(header_tbl); story.append(Spacer(1,14))
    score_color=C_GREEN if score>=70 else (C_ORANGE if score>=50 else C_RED)
    score_data=[[Paragraph(f"{score}",S_SCORE),[Paragraph(f"Grade {g}",S_GRADE),Paragraph(desc,S_CENTER),Spacer(1,6),Paragraph(f"Previous Score: {inp['Previous_Scores']}%",S_CENTER)]]]
    score_tbl=Table(score_data,colWidths=[usable*0.38,usable*0.62])
    score_tbl.setStyle(TableStyle([("BACKGROUND",(0,0),(-1,-1),C_LIGHT),("VALIGN",(0,0),(-1,-1),"MIDDLE"),("ALIGN",(0,0),(0,-1),"CENTER"),("LEFTPADDING",(0,0),(-1,-1),16),("RIGHTPADDING",(0,0),(-1,-1),16),("TOPPADDING",(0,0),(-1,-1),14),("BOTTOMPADDING",(0,0),(-1,-1),14),("BOX",(0,0),(-1,-1),1,C_BORDER),("LINEAFTER",(0,0),(0,-1),1,C_BORDER)]))
    story.append(score_tbl); story.append(Spacer(1,14))
    story.append(Paragraph("Student Information",S_H1)); story.append(HRFlowable(width="100%",thickness=1,color=C_BORDER,spaceAfter=6))
    info_items=[("Full Name",user_data.get("name","-")),("Username",user_data.get("username","-")),("Role",user_data.get("role","-").capitalize()),("Class",user_data.get("class","-")),("Gender",user_data.get("gender","-")),("Date of Birth",user_data.get("dob","-"))]
    info_rows=[]
    for i in range(0,len(info_items),2):
        l1,v1=info_items[i]; l2,v2=info_items[i+1] if i+1<len(info_items) else ("","")
        info_rows.append([Paragraph(l1,S_LABEL),Paragraph(v1,S_VALUE),Paragraph(l2,S_LABEL),Paragraph(v2,S_VALUE)])
    info_tbl=Table(info_rows,colWidths=[usable*0.18,usable*0.32,usable*0.18,usable*0.32])
    info_tbl.setStyle(TableStyle([("ROWBACKGROUNDS",(0,0),(-1,-1),[C_LIGHT,C_WHITE]),("LEFTPADDING",(0,0),(-1,-1),8),("RIGHTPADDING",(0,0),(-1,-1),8),("TOPPADDING",(0,0),(-1,-1),6),("BOTTOMPADDING",(0,0),(-1,-1),6),("BOX",(0,0),(-1,-1),1,C_BORDER),("INNERGRID",(0,0),(-1,-1),0.5,C_BORDER)]))
    story.append(info_tbl); story.append(Spacer(1,14))
    story.append(Paragraph("Prediction Parameters",S_H1)); story.append(HRFlowable(width="100%",thickness=1,color=C_BORDER,spaceAfter=6))
    param_items=[("Hours Studied",f"{inp['Hours_Studied']} hrs/day"),("Attendance",f"{inp['Attendance']}%"),("Previous Score",f"{inp['Previous_Scores']}%"),("Sleep Hours",f"{inp['Sleep_Hours']} hrs/day"),("Motivation Level",inp["Motivation_Level"]),("Teacher Quality",inp["Teacher_Quality"]),("School Type",inp["School_Type"]),("Internet Access",inp["Internet_Access"]),("Family Income",inp["Family_Income"]),("Parental Involvement",inp["Parental_Involvement"]),("Parent Education",inp["Parental_Education_Level"]),("Peer Influence",inp["Peer_Influence"]),("Learning Resources",inp["Learning_Resources"]),("Extracurricular",inp["Extracurricular_Activities"])]
    param_rows=[]
    for i in range(0,len(param_items),2):
        l1,v1=param_items[i]; l2,v2=param_items[i+1] if i+1<len(param_items) else ("","")
        param_rows.append([Paragraph(l1,S_LABEL),Paragraph(v1,S_VALUE),Paragraph(l2,S_LABEL),Paragraph(v2,S_VALUE)])
    param_tbl=Table(param_rows,colWidths=[usable*0.22,usable*0.28,usable*0.22,usable*0.28])
    param_tbl.setStyle(TableStyle([("ROWBACKGROUNDS",(0,0),(-1,-1),[C_LIGHT,C_WHITE]),("LEFTPADDING",(0,0),(-1,-1),8),("RIGHTPADDING",(0,0),(-1,-1),8),("TOPPADDING",(0,0),(-1,-1),5),("BOTTOMPADDING",(0,0),(-1,-1),5),("BOX",(0,0),(-1,-1),1,C_BORDER),("INNERGRID",(0,0),(-1,-1),0.5,C_BORDER)]))
    story.append(param_tbl); story.append(Spacer(1,14))
    story.append(Paragraph("Personalised Recommendations",S_H1)); story.append(HRFlowable(width="100%",thickness=1,color=C_BORDER,spaceAfter=8))
    sug_rows=[[Paragraph(f"{i+1}.  {tip}",S_TIP)] for i,tip in enumerate(sugs)]
    sug_tbl=Table(sug_rows,colWidths=[usable])
    sug_tbl.setStyle(TableStyle([("ROWBACKGROUNDS",(0,0),(-1,-1),[C_LIGHT,C_WHITE]),("LEFTPADDING",(0,0),(-1,-1),10),("RIGHTPADDING",(0,0),(-1,-1),10),("TOPPADDING",(0,0),(-1,-1),6),("BOTTOMPADDING",(0,0),(-1,-1),6),("BOX",(0,0),(-1,-1),1,C_BORDER),("INNERGRID",(0,0),(-1,-1),0.5,C_BORDER)]))
    story.append(sug_tbl); story.append(PageBreak())
    story.append(Paragraph("Performance Charts",S_H1)); story.append(HRFlowable(width="100%",thickness=1,color=C_BORDER,spaceAfter=10))
    charts=[]
    ...
    for title,cb in charts:
        ...
        try:
            img_io=io.BytesIO(cb); rl_img=RLImage(img_io,width=usable*0.92,height=usable*0.48)
            story.append(rl_img)
        except: story.append(Paragraph("(Chart could not be rendered)",S_BODY))
        story.append(Spacer(1,16))
    story.append(Spacer(1,10)); story.append(HRFlowable(width="100%",thickness=0.5,color=C_BORDER,spaceAfter=6))
    story.append(Paragraph("ScoreVision AI  |  AI-Powered Student Performance Analysis Platform  |  Confidential",S_FOOTER))
    doc.build(story)
    return buf.getvalue()
...
def page_landing(t):
    ...
    <div class="lp-hero">
      ...
      <div style="position:relative;z-index:2;max-width:680px;padding:40px 2rem;text-align:left">
        <div class="lp-eyebrow">
          <span style="width:6px;height:6px;border-radius:50%;background:#34d399;display:inline-block;box-shadow:0 0 8px #34d399;flex-shrink:0"></span>
          AI-Powered Academic Intelligence
        </div>
        <div class="lp-headline">Predict Your Exam Score<br><span>Before the Exam</span></div>
        <div class="lp-sub">
    ...
    with lc:
        if st.button("🚀 Get Started Free", use_container_width=True, key="lp_signup"):
            goto("signup")
    with mc:
        if st.button("🔑 Sign In", use_container_width=True, key="lp_login"):
            goto("login")
    with rc:
        if st.button("🔮 See a Demo", use_container_width=True, key="lp_demo"):
            goto("login")
    ...
    <div id="features" class="lp-section">
      ...
      <div class="lp-section-sub">A complete academic intelligence platform — not just a calculator. ScoreVision analyses your habits, environment, and history to deliver meaningful, actionable insights.</div>
    </div>""", unsafe_allow_html=True)

    feats = [
        ("🤖","ML Score Prediction","Linear Regression model trained on 6,000+ student records across 14 behavioural and environmental factors for accurate, personalised forecasts."),
        ("📊","Visual Analytics","Radar charts, progress gauges, score history trends, and metric breakdowns — all updating in real-time as you enter your inputs."),
        ("📄","Professional PDF Reports","Download a polished, multi-page PDF with your score, grade, input summary, charts, and personalised improvement suggestions."),
        ("📱","WhatsApp Sharing","Share your result card with parents, teachers, or friends directly via WhatsApp with a pre-formatted summary message."),
    ...
    steps = [
        ("1","Create Your Account","Sign up in under 30 seconds. Choose student or parent role and set up your profile with class, school, and personal details."),
        ("2","Enter Your Academic Details","Fill in 14 key parameters: study hours, attendance, sleep, motivation, teacher quality, peer influence, and more."),
        ("3","Get Your Personalised Report","Instantly receive your predicted score, grade, visual analytics, and a personalised action plan to maximise improvement."),
    ]
    with hw_l:
        for num,title,desc in steps:
            ...
            <div class="lp-step">
              ...
              <div><div class="lp-step-title">{title}</div><div class="lp-step-desc">{desc}</div></div>
            </div>""", unsafe_allow_html=True)

    with hw_r:
        factor_tags = "".join(
            f'<div style="background:{t["SURF2"]};border:1px solid {t["BORDER"]};border-radius:10px;padding:12px 14px;font-size:12px;color:{t["TX2"]};font-weight:600">{x}</div>'
            for x in ["Study Hours","Attendance","Sleep Quality","Motivation","Teacher Quality","Peer Influence","Family Income","Internet Access"]
        )
        st.markdown(f"""
        <div class="sv-card" style="padding:32px;margin-top:0">
          <div style="text-align:center;margin-bottom:20px">
            <div style="font-size:56px">🔭</div>
            <div style="font-family:'Sora',sans-serif;font-size:20px;font-weight:800;color:{t['TX']};margin-top:10px">Smart. Fast. Accurate.</div>
            <div style="font-size:13px;color:{t['TX2']};margin-top:6px;line-height:1.6;max-width:320px;margin-left:auto;margin-right:auto">
              Our ML model analyses the same factors teachers and researchers have identified as key predictors of exam performance.</div>
          </div>
          <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px">{factor_tags}</div>
        </div>""", unsafe_allow_html=True)
    ...
    <div id="testimonials" class="lp-section">
      ...
      <div class="lp-section-sub">Real feedback from the students and families who use ScoreVision AI every day.</div>
    </div>""", unsafe_allow_html=True)

    testis = [
        ("I predicted a 78 before my boards and scored 81. The study tips were spot on!","Priya S.","Class 12, Delhi"),
    ...
    <div class="lp-cta-band">
      ...
      <div class="lp-cta-band-sub">Join thousands of students already using ScoreVision AI.<br>Free to use. No credit card required.</div>
    </div>""", unsafe_allow_html=True)

    _,bc,_ = st.columns([2,1,2])
    with bc:
        if st.button("🚀 Start For Free — It's Quick!", use_container_width=True, key="lp_cta_bottom"):
            goto("signup")

    st.markdown(f"""
    <div style="text-align:center;padding:28px 0;font-size:12px;color:{t['TX3']};border-top:1px solid {t['BORDER']};margin-top:40px">
      ScoreVision AI &copy; {datetime.date.today().year} &nbsp;&bull;&nbsp; Built with Machine Learning &nbsp;&bull;&nbsp; Your data stays private
    </div>""", unsafe_allow_html=True)
...
def page_signup(t):
    ...
    with mc:
        ...
        if role == "Student":
            ...
            with c8:
                school_name = st.text_input("School / College", placeholder="e.g. DAV Jamshedpur")
        else:
            std_class   = "Parent"
...
def page_dashboard(t):
    ...
    st.markdown(f"""
    <div class="sv-card" style="background:linear-gradient(135deg,{t['GRAD1']}22,{t['GRAD2']}22);border-color:{t['GRAD1']}44;padding:32px;position:relative;overflow:hidden">
      ...
      <div style="font-family:'Sora',sans-serif;font-size:30px;font-weight:800;color:{t['TX']};letter-spacing:-0.02em">
        {name} {'🎓' if role=='student' else '👨‍👩‍👦'}</div>
      <div style="color:{t['TX2']};font-size:13px;margin-top:6px">
        {u.get('class','')}{'&nbsp;&bull;&nbsp;'+u.get('school','') if u.get('school') else ''}</div>
    ...
    for col,(v,l) in zip(tile_cols,[
        ...
        with col:
            st.markdown(f'<div class="sv-tile"><div class="v">{v}</div><div class="l">{l}</div></div>', unsafe_allow_html=True)

    st.markdown("<div style='height:16px'></div>", unsafe_allow_html=True)
    col_a,col_b = st.columns([3,2])

    with col_a:
        st.markdown(f'<div class="sv-sh">Score History</div><div class="sv-ss">Your last 10 predictions</div>', unsafe_allow_html=True)
        if preds:
            ...
            fig.add_trace(go.Scatter(x=dates,y=scores,mode="lines+markers",
                ...
                marker=dict(size=8,color=t["GRAD2"],line=dict(color=t["SURF"],width=2)),
                fill="tozeroy",fillcolor="rgba(56,189,248,0.08)",
                hovertemplate="<b>%{y}%</b><extra></extra>"))
            fig.update_layout(paper_bgcolor="rgba(0,0,0,0)",plot_bgcolor="rgba(0,0,0,0)",
                font_color=t["TX"],height=230,margin=dict(l=0,r=0,t=10,b=0),
                xaxis=dict(showgrid=False,color=t["TX2"]),
                yaxis=dict(showgrid=True,gridcolor=t["BORDER"],range=[0,105],color=t["TX2"]),
            ...
            <div class="sv-card" style="text-align:center;padding:40px">
              ...
              <div style="color:{t['TX3']};font-size:13px;margin-top:4px">Run your first prediction to see your progress</div>
            </div>""", unsafe_allow_html=True)
    ...
    with col_b:
        ...
        if photo:
            ...
            st.markdown(f'<img src="data:image/png;base64,{b64}" style="border-radius:50%;border:3px solid {t["AC"]};width:80px;height:80px;object-fit:cover;display:block;margin-bottom:12px">', unsafe_allow_html=True)
        else:
            initials="".join([x[0].upper() for x in name.split()[:2]])
            st.markdown(f"""<div style="width:80px;height:80px;border-radius:50%;background:linear-gradient(135deg,{t['GRAD1']}33,{t['GRAD2']}33);border:3px solid {t['AC']};display:flex;align-items:center;justify-content:center;font-size:28px;font-weight:800;color:{t['AC']};font-family:'Sora',sans-serif;margin-bottom:14px">{initials}</div>""", unsafe_allow_html=True)
        for lbl,val in [("Name",name),("Class",u.get("class","—")),("Gender",u.get("gender","—")),("DOB",u.get("dob","—")),("Role",u.get("role","—").capitalize())]:
            st.markdown(f"""<div style="display:flex;justify-content:space-between;padding:8px 0;border-bottom:1px solid {t['BORDER']};font-size:13px">
...
def page_predict(t):
    topbar(t)
    st.markdown('<div class="sv-sh">🔮 Predict Your Score</div><div class="sv-ss">Fill in your academic details for an AI-powered forecast</div>', unsafe_allow_html=True)

    with st.form("pred_form"):
        st.markdown('<div class="sv-card">', unsafe_allow_html=True)
        st.markdown(f"<div style='font-size:16px;font-weight:700;color:{t['TX']};margin-bottom:12px'>📚 Academic Details</div>", unsafe_allow_html=True)
        ...
        st.markdown(f"<hr class='sv-e'><div style='font-size:16px;font-weight:700;color:{t['TX']};margin-bottom:12px'>🌍 Environmental Factors</div>", unsafe_allow_html=True)
...
def page_results(t):
    ...
    with col_hero:
        ...
        <div class="sv-card" style="text-align:center;padding:40px 24px">
          <div style="font-size:13px;color:{t['TX2']};font-weight:600;letter-spacing:.1em;text-transform:uppercase;margin-bottom:20px">Predicted Score</div>
          <div class="sv-ring">
          ...
          <div style="margin-top:24px">
            <span class="sv-pill {pill}" style="font-size:15px;padding:9px 26px">Grade {g} — {desc}</span>
          </div>
        </div>""", unsafe_allow_html=True)

    with col_info:
        if score>=80:   st.markdown(f'<div class="sv-ag">Outstanding! You achieved {desc} performance.</div>', unsafe_allow_html=True)
        elif score>=60: st.markdown(f'<div class="sv-ao">{desc} performance. Keep pushing — you\'re close!</div>', unsafe_allow_html=True)
        else:           st.markdown(f'<div class="sv-ar">Predicted score is {score}%. Review suggestions below.</div>', unsafe_allow_html=True)
        ...
        for label,val,pct in [
            ...
            <div style="margin-bottom:14px">
              <div style="display:flex;justify-content:space-between;font-size:13px;font-weight:600;color:{t['TX']};margin-bottom:5px">
                <span>{label}</span><span style="color:{color}">{val}</span>
              </div>
              <div class="sv-progress-track">
                <div class="sv-progress-fill" style="width:{pct_c}%;background:linear-gradient(90deg,{color},{color}88)"></div>
              </div>
            </div>""", unsafe_allow_html=True)
        st.markdown('</div>', unsafe_allow_html=True)
    ...
    with col1:
        st.markdown(f'<div class="sv-sh">Factor Radar</div><div class="sv-ss">Environmental factors at a glance</div>', unsafe_allow_html=True)
        fm={"Motivation":{"Low":25,"Medium":60,"High":90},"Teacher":{"Poor":25,"Average":60,"Good":90},
            "Peer Inf.":{"Negative":20,"Neutral":55,"Positive":85},"Resources":{"Low":25,"Medium":60,"High":90},
            "Internet":{"No":30,"Yes":80},"Involvement":{"Low":25,"Medium":60,"High":90}}
        cats=list(fm.keys())
        vals=[fm["Motivation"].get(inp["Motivation_Level"],50),fm["Teacher"].get(inp["Teacher_Quality"],50),
              fm["Peer Inf."].get(inp["Peer_Influence"],50),fm["Resources"].get(inp["Learning_Resources"],50),
              fm["Internet"].get(inp["Internet_Access"],50),fm["Involvement"].get(inp["Parental_Involvement"],50)]
        fig1=go.Figure(go.Scatterpolar(r=vals+[vals[0]],theta=cats+[cats[0]],fill="toself",
            fillcolor="rgba(56,189,248,0.15)",line=dict(color=t["GRAD1"],width=2.5),
            marker=dict(color=t["GRAD2"],size=7)))
        fig1.update_layout(
            polar=dict(radialaxis=dict(visible=True,range=[0,100],color=t["TX2"],gridcolor=t["BORDER"]),
                       angularaxis=dict(color=t["TX2"]),bgcolor="rgba(0,0,0,0)"),
            paper_bgcolor="rgba(0,0,0,0)",font_color=t["TX"],height=300,
            margin=dict(l=30,r=30,t=20,b=20),showlegend=False)
        st.plotly_chart(fig1,use_container_width=True)

    with col2:
        st.markdown(f'<div class="sv-sh">Score Breakdown</div><div class="sv-ss">Key numeric metrics compared</div>', unsafe_allow_html=True)
        bv=[inp["Hours_Studied"]/8*100,inp["Attendance"],inp["Previous_Scores"],inp["Sleep_Hours"]/8*100]
        bc_list=[t["GR"] if v>=70 else (t["GO"] if v>=45 else t["RD"]) for v in bv]
        fig2=go.Figure(go.Bar(x=["Study Hrs","Attendance","Prev Score","Sleep"],y=bv,
            marker_color=bc_list,text=[f"{v:.0f}%" for v in bv],textposition="outside",
        ...
        fig2.update_layout(paper_bgcolor="rgba(0,0,0,0)",plot_bgcolor="rgba(0,0,0,0)",
            ...
            xaxis=dict(showgrid=False,color=t["TX2"]),showlegend=False,bargap=0.4)
        st.plotly_chart(fig2,use_container_width=True)

    st.markdown(f'<div class="sv-sh">Performance Gauge</div><div class="sv-ss">Predicted vs previous score of {inp["Previous_Scores"]}%</div>', unsafe_allow_html=True)
    fig3=go.Figure(go.Indicator(
        ...
        gauge={"axis":{"range":[0,100],"tickcolor":t["TX2"],"tickfont":{"color":t["TX2"]}},
               "bar":{"color":t["GRAD1"],"thickness":0.22},"bgcolor":"rgba(0,0,0,0)","borderwidth":0,
               "steps":[{"range":[0,50],"color":t["RDBG"]},{"range":[50,70],"color":t["GOBG"]},{"range":[70,100],"color":t["GRBG"]}],
               "threshold":{"line":{"color":t["GRAD2"],"width":4},"thickness":0.8,"value":score}},
        number={"font":{"color":t["AC"],"size":52,"family":"Sora"}},
        title={"text":f"Predicted vs Previous ({inp['Previous_Scores']}%)","font":{"color":t["TX2"],"size":13}}))
    fig3.update_layout(paper_bgcolor="rgba(0,0,0,0)",font_color=t["TX"],height=290,margin=dict(l=20,r=20,t=20,b=10))
    st.plotly_chart(fig3,use_container_width=True)

    st.markdown("<div style='height:16px'></div>", unsafe_allow_html=True)
    st.markdown(f'<div class="sv-sh">Personalised Suggestions</div><div class="sv-ss">AI-powered recommendations to boost your score</div>', unsafe_allow_html=True)
    for tip in sugs:
    ...
    st.markdown("<div style='height:16px'></div>", unsafe_allow_html=True)
    st.markdown(f'<div class="sv-sh">Share &amp; Download</div><div class="sv-ss">Export your full report or share on WhatsApp</div>', unsafe_allow_html=True)
    ...
    with col_wa:
        ...
        <a href="{wa_url}" target="_blank" style="text-decoration:none;display:block">
          <div style="background:#25D366;color:#fff;border-radius:50px;padding:14px 28px;text-align:center;font-weight:700;font-size:14px;font-family:'Plus Jakarta Sans',sans-serif;cursor:pointer;box-shadow:0 4px 15px rgba(37,211,102,0.35);letter-spacing:0.02em">
            📱 Share on WhatsApp
          </div>
        </a>
        <div style="text-align:center;font-size:12px;color:{t['TX3']};margin-top:8px">Opens WhatsApp with your score summary ready to send</div>""",
        unsafe_allow_html=True)
...
def page_profile(t):
    ...
    with col_b:
        ...
        with c1:
            ...
            new_gender=st.selectbox("Gender",opts,index=opts.index(u.get("gender","Male")) if u.get("gender") in opts else 0)
        with c2:
            try: dob_val=datetime.date.fromisoformat(u.get("dob","2005-01-01"))
            except: dob_val=datetime.date(2005,1,1)
            new_dob=st.date_input("Date of Birth",value=dob_val,min_value=datetime.date(1960,1,1),max_value=datetime.date.today())
        if u.get("role")=="student":
            classes=["Class 6","Class 7","Class 8","Class 9","Class 10","Class 11","Class 12","Undergraduate","Postgraduate"]
        ...
        st.markdown(f"<hr class='sv-e'><div style='font-size:16px;font-weight:700;color:{t['TX']};margin-bottom:12px'>🔑 Change Password</div>", unsafe_allow_html=True)
